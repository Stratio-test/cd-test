-- Migrate jdbc agent to sscc connector using kerberos authentication
...
-- * vault_secret_name: the name of the secret containing kerberos specific authentication info
...
with
  ...
  modifications as (
    ...
    union
    select "SERDE_ID", 'stratiossccdriver' as "PARAM_KEY", 'com.stratio.connectors.ssccmssql.MssqlDriverKRB' as "PARAM_VALUE"
    from tables_to_migrate
    union
    select "SERDE_ID", 'keytab' as "PARAM_KEY", 'mocktab' as "PARAM_VALUE"
    from tables_to_migrate
    union
...
where ("PARAM_KEY" = 'stratiosecuritymode' and "PARAM_VALUE" ='custom_sscc') or
      ("PARAM_KEY" = 'stratiossccdriver' and "PARAM_VALUE" ='com.stratio.connectors.ssccmssql.MssqlDriverKRB') or
