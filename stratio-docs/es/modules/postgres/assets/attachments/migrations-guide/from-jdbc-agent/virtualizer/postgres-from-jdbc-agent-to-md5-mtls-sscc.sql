-- Migrate jdbc agent to sscc connector using md5 (user/password) + mtls authentication
...
--   * for example 'jdbc:postgresql://<server>:<port>/<my_database>%'
...
with
  ...
  modifications as (
  	...
  	union
  	select "SERDE_ID", 'stratiossccdriver' as "PARAM_KEY", 'com.stratio.connectors.ssccpostgresql.PostgreSQLDriverMD5' as "PARAM_VALUE"
  	from tables_to_migrate
  	union
  	select "SERDE_ID", 'sslEnabled' as "PARAM_KEY", 'true' as "PARAM_VALUE"
  	from tables_to_migrate
  	union
  	select "SERDE_ID", 'driver' as "PARAM_KEY", 'embedded.com.stratio.connectors.ssccpostgresql.org.postgresql.Driver' as "PARAM_VALUE"
  	from tables_to_migrate
    union
...
where ("PARAM_KEY" = 'stratiosecuritymode' and "PARAM_VALUE" ='custom_sscc') or
      ("PARAM_KEY" = 'stratiossccdriver' and "PARAM_VALUE" ='com.stratio.connectors.ssccpostgresql.PostgreSQLDriverMD5') or
      ("PARAM_KEY" = 'sslEnabled' and "PARAM_VALUE" ='true') or
      ("PARAM_KEY" = 'driver' and "PARAM_VALUE" ='embedded.com.stratio.connectors.ssccpostgresql.org.postgresql.Driver') or
