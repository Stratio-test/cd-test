...
--   * for example 'jdbc:sap://<server>:<port>/<my_database>%'
...
with
  ...
  modifications as (
  	...
  	union
  	select "SERDE_ID", 'stratiossccdriver' as "PARAM_KEY", 'com.stratio.connectors.ssccsaphana.SapHanaDriver' as "PARAM_VALUE"
  	from tables_to_migrate
    union
...
where ("PARAM_KEY" = 'stratiosecuritymode' and "PARAM_VALUE" ='custom_sscc') or
      ("PARAM_KEY" = 'stratiossccdriver' and "PARAM_VALUE" ='com.stratio.connectors.ssccsaphana.SapHanaDriver') or
