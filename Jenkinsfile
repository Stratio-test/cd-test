...
//                        remoteServer: "anchore-reports.int.stratio.com", localFolder: "anchore", branchOnPath: true])
  ...
  //      doSemgrepAnalysis(conf: config, configs: ["/semgrep-rules/rules/python-rules.yaml", "/semgrep-rules/rules/java-rules.yaml"], includes: ["src/*", "python/*.py"])
        ...
        doCustomStage(conf:config, buildToolOverride: [BUILDTOOL_IMAGE: "python:latest", CREDENTIALS_ID: [[credentialsId: "STRATIOCOMMIT-TEST_GH_API_TOKEN", credentialsVariable:"VAR1"], [credentialsId: "POSTGREST_JWT", credentialsVariable:"VAR2"]], CUSTOM_COMMAND: 'python python/test.py %%VERSION'], stageName: "Running python scripts", runOnFinal: true)
